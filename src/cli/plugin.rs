...
mod tests {
    ...
    fn github_plugin_source_parses_root_repo() {
        let source = GithubPluginSource::parse("example-owner/herdr-plugin-examples").unwrap();
        assert_eq!(source.owner, "example-owner");
        assert_eq!(source.repo, "herdr-plugin-examples");
        assert_eq!(source.subdir, None);
        assert_eq!(
            source.remote_url(),
            "https://github.com/example-owner/herdr-plugin-examples.git"
        );
    }
    ...
    fn github_plugin_source_parses_subdir() {
        let source =
            GithubPluginSource::parse("example-owner/herdr-plugin-examples/worktree-bootstrap")
                .unwrap();
        assert_eq!(source.owner, "example-owner");
        assert_eq!(source.repo, "herdr-plugin-examples");
    ...
    fn github_plugin_source_rejects_non_shorthand_sources() {
        for source in [
            "https://github.com/example-owner/herdr-plugin-examples",
            "git@github.com:example-owner/herdr-plugin-examples.git",
            "example-owner",
            "example-owner/herdr-plugin-examples/../bad",
            "example-owner/herdr-plugin-examples//bad",
        ] {
            assert!(
    ...
    fn github_source_lookup_matches_installed_plugin_source() {
        let source =
            GithubPluginSource::parse("example-owner/herdr-plugin-examples/agent-telegram-notify")
                .unwrap();
        let plugins = vec![
            github_plugin(
