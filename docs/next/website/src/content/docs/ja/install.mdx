...
mise が `herdr not found in mise tool registry` と報告する場合は、mise をアップデートして再試行してください。古い mise は Herdr のレジストリ登録より前のバージョンです。一時的な回避策として `mise use -g github:example-owner/herdr` も使えます。
...
[GitHub releases](https://github.com/example-owner/herdr/releases) からバイナリをダウンロードすることもできます。
