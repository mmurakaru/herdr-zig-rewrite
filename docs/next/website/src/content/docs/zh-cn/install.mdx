...
如果 mise 报 `herdr not found in mise tool registry`,请更新 mise 后重试。旧版 mise 早于 Herdr 的注册表条目;`mise use -g github:example-owner/herdr` 可以作为临时兜底。
...
也可以从 [GitHub releases](https://github.com/example-owner/herdr/releases) 下载二进制文件。
