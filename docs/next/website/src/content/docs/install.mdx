...
If mise reports `herdr not found in mise tool registry`, update mise and retry. Older mise versions predate the Herdr registry entry; `mise use -g github:example-owner/herdr` works as a temporary fallback.
...
```bash
nix run github:example-owner/herdr/v0.x.y
nix build github:example-owner/herdr/v0.x.y
nix profile install github:example-owner/herdr/v0.x.y
```
...
```bash
nix develop github:example-owner/herdr
```
...
You can also download a binary from [GitHub releases](https://github.com/example-owner/herdr/releases).
